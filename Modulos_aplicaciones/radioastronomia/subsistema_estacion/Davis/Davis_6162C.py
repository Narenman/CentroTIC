...
def read():
        ...
        if len(data_packet)==100:
                ...
                out = {'presion_atomosferica': round((Barometer/1000.0)*3386.38/100,1),
                        'temperatura': round((OutSideTemp/10.0+459.67)*(5.0/9), 2), #round((OutSideTemp/10.0-32)*(5.0/9), 2), CELCIUS
                        'Inside Temperature': InsideTemp,
                        'humedad_relativa': OutSideHumi,
                        'dir_viento': str(WindDirect),
                        'vel_viento': WindSpeed,
                        'Wind AVG': AvgWSpeed,
                        'UV': UV,
                        'precipitacion': RainRate,
                        'radiacion_solar': SolarRadia}
        else:
                out = 0
